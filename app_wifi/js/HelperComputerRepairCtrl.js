angular.module('starter.HelperComputerRepairCtrl', [])

    .controller('HelperComputerRepairCtrl', function($scope,$state,service,$rootScope,$timeout,
        ...
        $scope.toHelperComputer = function () {
            $state.go('tab.HelperComputer')
        };
        $scope.toComputerHistory = function () {
            $state.go('tab.HelperComputerHistory')

        };
        //钉钉标题与返回键设置
        dd.ready(function() {
            dd.biz.navigation.setTitle({
                title : '终端报修',//控制标题文本，空字符串表示显示默认文本
